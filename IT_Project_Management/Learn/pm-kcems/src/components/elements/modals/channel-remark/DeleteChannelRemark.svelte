<script>
	...
	import { deleteChannelRemark } from '$providers/actions/kchannel/channel/channel';
	import { createEventDispatcher } from 'svelte';
   const dispatch = createEventDispatcher();
	export let remark;
	export let shown;
   let loadADeleteAttdSessionState = {
		deletedChannelRemark: null,
		loading: false,
		loaded: false,
		errMsg: '',
	};
	...
	const onLoadADeleteAttdSession = async () => {
		try {
			...
			const res = await deleteChannelRemark.load({
				id: remark.id,
			});
			loadADeleteAttdSessionState.deletedChannelRemark = res.data;
			loadADeleteAttdSessionState.loaded = true;
			dispatch('deleted', { deletedChannelRemark: res.data });
			...
			<div class="flex flex-col items-center h-full px-5 pt-6">
				...
				<div class="flex items-center justify-center p-3 text-xl text-gray-500">
					<div class=" text-center h-[62px] line-clamp-2">
						Are you sure you want to delete this Remark <span
							class="pr-1 text-primary-3">{remark.name}</span
						>?
					</div>
				</div>
